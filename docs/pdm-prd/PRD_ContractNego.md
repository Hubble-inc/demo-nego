# NegoSign by Hubble 製品仕様書（PRDインデックス）
...
NegoSignは2つの利用パターンがあり、PRDを分割して管理しています。
...
**パターン1: NegoSign単体利用** - NegoSignを単独で使う場合
...
**パターン2: Hubble連携利用** - HubbleからNegoSignを使う場合
- ユーザーフロー（Hubble内「交渉相手に共有」→招待→交渉→署名→Hubbleに反映）
- Hubble側の画面変化（hubble-ui-first-share.html, hubble-ui.html）
- 交渉バナー（「交渉中 | Ver.Xを共有中」）
- コメントの区別（社内 vs 交渉）
- バージョン同期（NegoSign→Hubble自動、Hubble→NegoSign手動共有）

---

## 2つの利用パターン早見表

| 項目 | NegoSign単体利用 | Hubble連携利用 |
|------|-------------|----------------|
| 対象ユーザー | Hubble未導入の企業・個人 | 既存Hubbleユーザー |
| 契約書のマスター | NegoSign | Hubble |
| 入口画面 | NegoSignダッシュボード | Hubbleの契約書画面 |
| 社内コメント | なし（全員に見える） | Hubble側で管理（社外に見えない） |
| バージョン管理 | NegoSign内で完結 | Hubbleと同期 |
