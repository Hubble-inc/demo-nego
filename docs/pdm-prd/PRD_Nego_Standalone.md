...
### 1.1 対象ユーザー
- 契約交渉ツールとしてNegoSignを使いたい企業・個人
- 取引先との契約書のやり取りを効率化したいビジネスパーソン

### 1.2 主な特徴
- 契約書をNegoSignに直接アップロード
- NegoSignが契約書の「マスター」となる
- 交渉相手はNegoSignに招待して参加
...
| 機能 | 理由 |
|------|------|
| 組織・チーム管理 | NegoSignは案件単位の交渉に特化する。組織管理は将来検討 |
| 承認ワークフロー | 同上 |
| 組織横断のダッシュボード | 同上 |
| 管理者ロール | 同上 |
| テンプレート管理 | MVPではテンプレート機能を提供しない。ユーザーは手元のWordファイルをアップロードして交渉を開始する。テンプレートのニーズが確認された場合は Phase 2 で検討 |

> 交渉単位でのメンバー追加（PRD_Common §8.3）は対応済みのため、「この交渉を同僚にも見せたい」は現行仕様で解決できる。

---

## 8. オンボーディング

初めてNegoSignにサインインしたユーザーが、最短で価値を体験できるようにする。
...
招待リンク経由で初めてNegoSignにアクセスするユーザーのフローと画面仕様。共通仕様はPRD_Common.md §8.6 を参照。

### 10.1 NegoSign単体利用における追加仕様
