...
- MVP の概要タブ: サマリー + ハイライトのみ表示。タイムラインと流れ図の欄は Phase 2 まで非表示（空欄やプレースホルダーは置かない）
...
| 項目 | 仕様 |
|------|------|
| デフォルトタブ | 概要タブ（AI要約が即座に目に入る状態。通常ユーザーのデフォルト「契約書」タブとは異なる） |
...
#### 契約書タブ
- 書類サブタブ（複数書類がある場合、書類ごとにタブ切り替え表示。「+ 追加」で書類の追加が可能）
- バージョン履歴パネル（左サイドバー）
- 契約書本文（中央パネル）
...
> **契約書タブとの違い**: 契約書タブは交渉のメイン書類（バージョン管理対象）。関連書類タブは補足資料の保管場所で、バージョン管理は行わない。
...
交渉詳細画面の右カラムに常設されるサイドバー。メインタブ（概要/契約書/関連書類）に依存せず常に表示される。
