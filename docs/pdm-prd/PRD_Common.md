...
> このドキュメントは、NegoSignの全機能に共通する仕様を定義します。
> - 画面・フロー仕様 → [PRD_Nego_Standalone.md](./PRD_Nego_Standalone.md)
...
> **設計意図**: テキスト選択 → 引用ブロック → メッセージ送信のフローにより、コメントが何に対するものかが常に明確になる。文脈を保った議論ができる設計。
...
| 受け手の状態に合わせる | 未登録ユーザー・登録ユーザーで通知内容と導線を最適化 |

### 7.2 ユーザータイプ別の通知対象

通知の受け手は2タイプ存在する。それぞれ受け取る通知の範囲と導線が異なる。
...
| イベント | トリガー | タイプA | タイプB | 優先度 | 配信タイミング |
|---------|---------|:------:|:------:|-------|------------|
...
|---------|---------|------|
| メール | 対応 | 全ての通知の基本チャネル |
| アプリ内通知パネル | 対応 | ログイン中のリアルタイム通知。§7.8を参照 |
| ブラウザ通知 | 将来対応 | ログイン中のプッシュ通知 |
| Slack / Teams連携 | 将来対応 | チーム利用時の通知先として |

---

### 7.7 個人設定（グローバル設定 / settings.html）
...
|---------|------|
| PDF 表紙 | ロゴ＋「NegoSign」 |
| PDF 全ページフッター | 「NegoSign」（小フォント） |
| CSV / JSONファイル | ブランディングなし |
| ZIP内のREADME.txt | 「この記録は NegoSign で生成されました」＋ website.html へのURL |
