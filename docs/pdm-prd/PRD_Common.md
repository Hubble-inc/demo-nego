...
> このドキュメントは、NegoSign単体利用・Hubble連携利用の両方に共通する仕様を定義します。
> - NegoSign単体利用の詳細 → [PRD_Nego_Standalone.md](./PRD_Nego_Standalone.md)
...
> **設計意図**: 法務担当者はWordの比較機能に習熟している。NegoSign内に差分ビューアを構築するよりも、AIによる「差分の意味の解説」に開発リソースを集中する方がユーザー価値が高い。
...
#### タイプA: NegoSignユーザー（Hubble未連携）

NegoSignに登録済みのユーザー。交渉の主体として全ての通知を受け取る。

#### タイプB: 未サインアップの招待ユーザー

NegoSignユーザーに招待されたが、まだサインアップしていないユーザー。メール通知のみ受け取り、通知の導線はすべて招待受領画面（invite.html）やサインアップへ誘導する。

#### タイプC: Hubble連携ユーザー

HubbleとNegoSignを連携しているユーザー。基本的にはタイプAと同じ通知を受け取るが、Hubble側の通知（社内コメント等）と重複しないよう調整する。
...
> ※ タイプCの「新しいコメント」：NegoSign上の交渉コメントのみ通知。Hubble側の社内コメントはHubbleの通知で管理。
...
まだNegoSignに登録していないユーザーへの通知は、行動を促す重要な接点となる。
...
| 項目 | 調整内容 |
|------|---------|
| コメント通知 | NegoSign上の交渉コメントのみ通知。Hubble側の社内コメント通知とは分離 |
| 契約書更新通知 | NegoSignから相手が更新した場合のみ通知。自社がHubbleから再共有した場合は通知しない（自分のアクションのため） |
...
§7.3 で定義されている全通知イベントのうち、タイプA（NegoSignユーザー）向けの通知をアプリ内にも表示する。メールとアプリ内通知は同じイベントで同時発行される。
...
> 署名者は交渉への招待とは別に、署名手続き・設定（セクション5.2）で個別に設定する。NegoSignに招待する時点では権限タイプは1種類（メンバー）のみ。
...
招待リンク経由で初めてNegoSignにアクセスするユーザーの体験設計。「呼ばれたから来た人」が最短で交渉に参加できることを最優先する。
...
NegoSign単体利用・Hubble連携利用のどちらでも共通する交渉詳細画面（negotiation.html）の仕様。
...
| 配置箇所 | 表現 |
|---------|------|
| PDF 表紙 | ロゴ＋「NegoSign by Hubble」 |
| PDF 全ページフッター | 「NegoSign by Hubble」（小フォント） |
| CSV / JSONファイル | ブランディングなし |
| ZIP内のREADME.txt | 「この記録は NegoSign by Hubble で生成されました」＋ website.html へのURL |
